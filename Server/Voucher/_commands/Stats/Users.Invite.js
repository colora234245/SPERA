...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    message.channel.send({
      embeds: [new genEmbed().setAuthor(member.user.username, member.user.avatarURL({ dynamic: true })).setDescription(`${member} isimli üye toplam \`${total + bonus}\` davete sahip. (\`${regular}\` giren, \`${leave}\` ayrılmış, \`${fake}\` sahte, \`${daily}\` günlük, \`${weekly}\` haftalık)
