...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    CategoryChannels.findOne({ channelID: args[0] }, async (err, data) => {
      if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen kategori kanalı geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.Iptal))
