...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.channel.send(cevaplar.üye + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@ramal/ID> <Sebep>\``);
    ...
    if(getLimit.get(message.member.id) >= ayarlar.banLimit) return message.reply(`${cevaplar.prefix} Underworld komutu kullanım hakkınız dolmuştur.`);
    ...
    if(sunucudabul) {
        ...
        await message.channel.send(`${message.guild.emojiGöster(emojiler.Yasaklandı)} ${uye.toString()} isimli üyeye \`${sebep}\` sebebiyle "__Underworld__" türünde ceza-i işlem uygulandı. (\`Ceza Numarası: #${cezano}\`)`)
