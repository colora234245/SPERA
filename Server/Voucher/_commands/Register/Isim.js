...
module.exports = {
    Isim: "isim",
    Komut: ["i","nick"],
    Kullanim: "isim <@ramal/ID> <İsim> <Yaş>",
  ...
  onRequest: async function (client, message, args) {
    if(!ayarlar.tag) return message.reply(cevaplar.ayarlamayok);
    ...
    if (yaş < ayarlar.minYaş) return message.reply(cevaplar.yetersizyaş).then(x => setTimeout(() => {x.delete()}, 7500))
    ...
    uye.setNickname(`${setName}`).catch(err => message.reply(cevaplar.isimapi));
    ...
    message.reply({embeds: [new genEmbed().setDescription(`${uye} üyesinin ismi "${isim} | ${yaş}" olarak değiştirildi${isimveri.Names ? isimveri.Names.length > 0 ? 
...
${isimler}\n\nÜyesinin önceki isimlerine \`${sistem.botSettings.Prefixs[0]}isimler <@ramal/ID>\` komutuyla bakarak kayıt işlemini\n gerçekleştirmeniz önerilir.`
: "." : "."}`)]}).then(msg => {
