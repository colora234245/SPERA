...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let embed = new MessageEmbed().setAuthor(sistem.embed.başlık, message.guild.iconURL({dynamic: true, size: 2048})).setFooter(sistem.embed.altbaşlık).setColor(sistem.embed.renk);
    let embed2 = new MessageEmbed().setAuthor(sistem.embed.başlık, message.guild.iconURL({dynamic: true, size: 2048})).setFooter(sistem.embed.altbaşlık).setColor(sistem.embed.renk);
    let embed3 = new MessageEmbed().setAuthor(sistem.embed.başlık, message.guild.iconURL({dynamic: true, size: 2048})).setFooter(sistem.embed.altbaşlık).setColor(sistem.embed.renk);
...
${x.UrunDetay}`, true)})
    ...
    if(!tepki) return;
      if (tepki.emoji.name === '💸') { 
        ...
        message.channel.awaitMessages(filter, {max: 1, time: 10000}).then(async ramal => { 
          let ürünid = ramal.first().content
          ...
          if(!alıncakürün) {
            ...
            .then(async ramalsatinal => { 
              if(ramalsatinal.first().content.toLowerCase() === "hayır" || ramalsatinal.first().content.toLowerCase() === "Hayır") {
              ...
              if(ramalsatinal.first().content.toLowerCase() === "evet" || ramalsatinal.first().content.toLowerCase() === "Evet") { 
