...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!res) return message.channel.send(`${cevaplar.prefix} Belirttiğin \`#${args[0]}\` numaralı ceza numarasında ceza bilgisi bulunamadı.`)
        // Cezalanan Üye
        ...
        • Ceza Süresi: \`${res.Duration ? moment.duration(res.Duration - res.Date).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika] ') : "Kalıcı"}\`${res.Active ? res.Duration ? ` / \`${moment.duration(res.Duration - Date.now()).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika,] s [Saniye]')} kaldı.\``: "" : ""}
