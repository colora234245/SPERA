      ...
      if(i.customId == "kullanacakRoller") {
        let roles = roller.rolPanelRolleri
          ...
          .then(async verilcekRole => {
            let ramalsiksinananı = verilcekRole.first()
          ...
          let rolPushing = []
          if(ramalsiksinananı.mentions.roles.size >= 1) {
            rolPushing = ramalsiksinananı.mentions.roles.map(role => role.id)
          } else {
            let argss = ramalsiksinananı.content.split(" ");
        ...
        collector1.on('collect', async i => { 
          if(i.customId == "komutSil") {
            let roles = roller.rolPanelRolleri
              ...
              const findCmd = komutBul.talentPerms.find(ramal => ramal.Commands == i.values[0]);
