...
${pages[currentPage - 1].map((value, index) => `\`${index + 1}.\` \`${global.sistem.botSettings.Prefixs[0] + value.Komut}\` ${value.Kanal ? message.guild.channels.cache.get(value.Kanal) ? `(#${message.guild.channels.cache.get(value.Kanal).name})` : `` : ``}  \`${tarihsel(value.Tarih)}\``).join("\n")} `)]}).catch(err => {})
