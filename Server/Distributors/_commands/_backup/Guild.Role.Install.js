...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    await roleBackup.findOne({ roleID: args[0] }, async (err, data) => {
      if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen rol geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.Iptal))
      ...
      await client.queryManage(args[0], newRole.id)
    }).catch(err => {});
