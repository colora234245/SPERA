...
module.exports = async (member) => {
    ...
    if (!channel) return;
   if (member.user.bot) return channel.send({ content: `${member.guild.emojiGöster(emojiler.Iptal)} **${member.user.tag}** isimli üye, sunucumuzdan ayrıldı!` })
    const inviteMemberData = await GUILD_INVITE.findOne({ userID: member.user.id }) || [];
    if (!inviteMemberData.Inviter) {
      return channel.send({ content: `${member.guild.emojiGöster(emojiler.Iptal)} **${member.user.tag}** isimli üye, sunucumuzdan ayrıldı!` });
      ...
      if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
        ...
        const total = inviterData ? inviterData.total : 0;  
        return channel.send({ content: `${member.guild.emojiGöster(emojiler.Iptal)} **${member.user.tag}** isimli üye, sunucumuzdan ayrıldı! **${inviter.tag}** (\`${total}\` daveti bulunmakta)` })
      } else {
