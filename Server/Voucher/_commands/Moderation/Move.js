...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.reply(cevaplar.üye + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@ramal/ID> <Kanal ID>\``);
    let kanal = message.guild.channels.cache.get(args[1]);
    if(!kanal) return message.reply(cevaplar.argümandoldur + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@ramal/ID> <Kanal ID>\``);
