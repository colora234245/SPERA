...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) message.react(message.guild.emojiGöster(emojiler.Iptal))
    ...
    Users.findOne({_id: uye.id }, async (err, res) => {
      ...
      await curPage.edit({embeds: [embed.setDescription(`${uye} isimli üyesinin toplamda \`${res.Staffs.length || 0}\` adet yetkilisi mevcut.
...
${pages[currentPage - 1].map((value, index) => `\`${index + 1}.\` ${message.guild.members.cache.get(value.id) ? message.guild.members.cache.get(value.id) : `<@${value.id}>`}  \`${tarihsel(value.Date)}\``).join("\n")} `)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${uye} isimli üyesinin toplamda \`${res.Staffs.length || 0}\` adet yetkilisi mevcut.
