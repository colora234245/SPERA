...
module.exports = {
    Isim: "kayıt",
    Komut: ["kay","k"],
    Kullanim: "kayıt @ramal/ID <isim> <yaş>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.reply(cevaplar.üye + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@ramal/ID> <Isim> <Yaş>\``);
    ...
    if(!uye.manageable) return message.reply(cevaplar.dokunulmaz).then(x => { setTimeout(() => {x.delete()}, 5000)})
    if(roller.erkekRolleri.some(x => uye.roles.cache.has(x))) return message.reply(cevaplar.kayıtlı);
    if(roller.kadınRolleri.some(x => uye.roles.cache.has(x))) return message.reply(cevaplar.kayıtlı);
    if(message.member.roles.highest.position <= uye.roles.highest.position) return message.reply(cevaplar.yetkiust).then(x => { setTimeout(() => {x.delete()}, 5000)})
    if(ayarlar.teyitZorunlu && !uye.voice.channel && !uye.roles.cache.has(roller.vipRolü) && !message.member.permissions.has('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku))) return message.channel.send(cevaplar.teyitzorunlu).then(x => { setTimeout(() => {x.delete()}, 5000)});    
    ...
    if (yaş < ayarlar.minYaş) return message.reply(cevaplar.yetersizyaş).then(x => { setTimeout(() => {x.delete()}, 5000)})
    if(!isim || !yaş) return message.reply(cevaplar.argümandoldur + ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <@ramal/ID> <Isim> <Yaş>\``);
