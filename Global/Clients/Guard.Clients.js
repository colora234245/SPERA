        ...
        async queryManage(oldData, newData) {
            ...
            if(sunucuData) {              
                ...
                if(oldData === sunucuData.voicemuteRolü) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.voicemuteRolü": newData}})
                }
                ...
                if(sunucuData.banKoru.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.banKoru": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.banKoru": newData}})
                }
                ...
                if(sunucuData.teleportHammer.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.teleportHammer": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.teleportHammer": newData}})
                }
                ...
                if(sunucuData.abilityHammer.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.abilityHammer": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.abilityHammer": newData}})
                }
        ...
        connect(token) {
            ...
            this.login(token).then(ramal => {
                ...
                this.on("ready", async () => { 
                    let Whitelist = await GUARD_SETTINGS.findOne({guildID: sistem.SERVER.ID})
                    if(!Whitelist) {
                        await GUARD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, {$set: {"auditLimit": 10, auditInLimitTime: "2m"}}, {upsert: true})
                    } 
