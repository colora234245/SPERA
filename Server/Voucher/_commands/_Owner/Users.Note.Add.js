...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    message.channel.send({embeds: [new genEmbed().setDescription(`${message.guild.emojiGöster(emojiler.Onay)} ${uye} isimli üyeye \`${not}\` notu **${tarihsel(Date.now())}** tarihinde eklendi.`)]}).then(msg => {
        setTimeout(() => {
            msg.delete()
        }, 7500);
    })
