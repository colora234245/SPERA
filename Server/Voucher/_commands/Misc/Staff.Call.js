...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if(!ayarlar.staff.includes(message.member.id) && !message.member.permissions.permissions.has('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku))) return message.react(message.guild.emojiGöster(emojiler.Iptal));
    let embed = new genEmbed()
    let enAltYetkiliRolü = message.guild.roles.cache.get(roller.altilkyetki);
