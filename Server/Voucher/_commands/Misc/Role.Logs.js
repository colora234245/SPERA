  ...
  onRequest: async function (client, message, args) {
    ...
    Users.findOne({_id: uye.id }, async (err, res) => {
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${pages[currentPage - 1].map((x, index) => `\`${x.state == "Ekleme" ? "✅" : "❌"}\` ${Array.isArray(x.rol) ? x.rol.map(x => message.guild.roles.cache.get(x)).join(", ") : message.guild.roles.cache.get(x.rol)} (\`${tarihsel(x.tarih)}\`) (<@${x.mod ? x.mod : "Bilinmeyen Yetkili"}>)`).join("\n")}`)]
        }).catch(err => {});
