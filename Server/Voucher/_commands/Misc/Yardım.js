...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let collector = ramal.createMessageComponentCollector({filter: filter, time: 30000, error: ["time"]})
    ...
    collector.on("collect", i => {
        if(i.customId == "yardimmenusu") {
           if(i.values == "talent") {
                ...
                ramal.edit({embeds: [new genEmbed().setDescription(`${client.commands.filter(x => x.Extend != false && x.Kategori == `${i.values}`).map(x => `\`${sistem.botSettings.Prefixs[0] + x.Kullanim}\``).join("\n")}`)]})
           }
           i.deferUpdate()
        }
    })
    collector.on("end", () => {
        ramal.edit({embeds: [new genEmbed().setDescription(`${client.commands.filter(x => x.Extend != false).map(x => `\`${sistem.botSettings.Prefixs[0] + x.Kullanim}\``).join("\n")}
        \`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => sistem.botSettings.Prefixs[0] + x.Commands + " <@ramal/ID>").join("\n") : '' : ''}\``)], components: []})
      })
    }
};